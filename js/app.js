...
var TAG_META={
  '\u9ed1\u5343\u5c42':{icon:'ð',title:'\u9ed1\u5343\u5c42'},
  '\u767d\u5343\u5c42':{icon:'ð',title:'\u767d\u5343\u5c42'},
  '\u53f6\u7247':{icon:'ð',title:'\u53f6\u7247'},
  '\u6574\u809a':{icon:'ð',title:'\u6574\u809a'},
  '\u8fb9\u89d2\u6599':{icon:'ð',title:'\u8fb9\u89d2\u6599'},
  '\u867e\u6ed1':{icon:'ð¦',title:'\u867e\u6ed1'},
  '\u5176\u4ed6':{icon:'ð¦',title:'\u5176\u4ed6'}
};
...
async function renderHome(c){
  isLoading=true;
  c.innerHTML='<div style="text-align:center;padding:60px 20px;color:#999"><div style="font-size:32px;margin-bottom:10px">â³</div>å è½½ä¸­...</div>';
  
  var allProducts=[];
  try{allProducts=await getProducts();}catch(e){allProducts=[];console.error("[DEBUG] getProducts error:", e);}
  ...
  sidebarH+='<button class="sidebar-btn'+(activeCategory===''?' active':'')+'" onclick="setCategory(\'\')"><span class="sidebar-icon">ð</span><span class="sidebar-label">\u5168\u90e8</span><span class="sidebar-count">'+totalCount+'</span></button>';
  for(var ci=0;ci<TAG_ORDER.length;ci++){
    var tag=TAG_ORDER[ci],cnt=byTag[tag]?byTag[tag].length:0;
    if(cnt===0)continue;
    var meta=TAG_META[tag]||{icon:'ð¦',title:tag};
  ...
  if(activeCategory===''){
    ...
    if(Object.keys(byTag).length===0){contentW+='<div class="empty"><span class="empty-icon">ð¦</span><p>\u6682\u65e0\u5546\u54c1</p><p style="font-size:12px;color:#bbb;margin-top:10px">\u8bf7\u5230\u7ba1\u7406\u9875\u9762\u6dfb\u52a0\u5546\u54c1</p></div>';}
  } else {
    var arr=byTag[activeCategory]||[];
    if(arr.length>0){var meta=TAG_META[activeCategory]||{icon:'ð¦',title:activeCategory};contentW+=renderSec(meta.icon,meta.title,arr);}
    else{contentW+='<div class="empty"><span class="empty-icon">ð¦</span><p>\u8be5\u5206\u7c7b\u6682\u65e0\u5546\u54c1</p></div>';}
  }
  contentW+='</div>';
...
async function showDetail(id){
  ...
  var backBtn=activeCategory?
    '<button class="detail-btn" onclick="setCategory(\''+activeCategory+'\')" style="background:#f0f0f0;color:#333;margin-bottom:10px">â '+activeCategory+'</button>':
    '<button class="detail-btn" onclick="switchTab(\'home\')" style="background:#f0f0f0;color:#333;margin-bottom:10px">â \u8fd4\u56de\u9996\u9875</button>';
  var h='';
  if(p.video){
    ...
    h+='<div class="detail-video-play-btn" id="detailPlayOverlay" onclick="toggleDetailVideo()"><span class="play-icon">â¶</span></div>';
    h+='<button class="video-replay-btn" id="detailReplayBtn" onclick="replayVideo()" style="display:none">ð \u91cd\u65b0\u64ad\u653e</button>';
    h+='<button class="video-fullscreen-btn" onclick="toggleVideoFullscreen()">â¶ \u5168\u5c4f</button>';
    h+='</div><div class="detail-video-label">ð¥ \u5546\u54c1\u89c6\u9891</div></div>';
  }
  if(allImgs.length>0){
  ...
  var unit=p.unit||'\u4ef6';
  h+='<div class="detail-content">'+backBtn+'<div class="detail-name">'+p.name+'</div><div class="detail-price">Â¥'+p.price+'<span class="price-unit">/'+unit+'</span></div>';
  if(p.tag)h+='<span class="detail-tag">'+p.tag+'</span>';
  if(p.description)h+='<div class="detail-desc">'+p.description+'</div>';
  h+='<div class="detail-actions">';
  h+='<button class="detail-btn share" onclick="shareProduct('+p.id+')">ð¤ \u5206\u4eab</button>';
  h+='<button class="detail-btn copy" onclick="copyPrice('+p.price+')">ð \u590d\u5236\u4ef7\u683c</button>';
  h+='</div><div style="margin-top:15px"><button class="detail-btn copy" onclick="switchTab(\'home\')" style="width:100%">â \u8fd4\u56de\u9996\u9875</button></div></div>';
...
function copyPrice(v){var i=document.createElement('input');i.value=v;document.body.appendChild(i);i.select();document.execCommand('copy');document.body.removeChild(i);showToast('\u5df2\u590d\u5236\u4ef7\u683c Â¥'+v);}
async function shareProduct(id){var p=await getProductById(id),t=p?p.name+' - Â¥'+p.price:'\u98df\u6750\u5c55\u793a',u=window.location.href.split('?')[0]+'?id='+id;if(navigator.share)navigator.share({title:t,text:p?p.description:'\u65b0\u9c9c\u597d\u98df\u6750',url:u}).catch(function(){});else{copyToClipboard(u);showToast('\u94fe\u63a5\u5df2\u590d\u5236\uff0c\u53ef\u53d1\u9001\u7ed9\u597d\u53cb');}}
...
function renderPwd(c){
  c.innerHTML='<div class="password-modal"><div class="password-box"><div class="password-title">ð \u7ba1\u7406\u5458\u9a8c\u8bc1</div><input type="password" class="password-input" id="pwdInput" placeholder="\u8bf7\u8f93\u5165\u5bc6\u7801" maxlength="20"><button class="password-btn" onclick="verifyPwd()">\u8fdb\u5165\u7ba1\u7406</button></div></div>';
...
async function renderAdmin(c){
  c.innerHTML='<div style="text-align:center;padding:40px;color:#999">â³ \u52a0\u8f7d\u4e2d...</div>';
  var ps=await getProducts();
  ps.sort(function(a,b){return(parseFloat(a.price)||0)-(parseFloat(b.price)||0);});
  var h='<button class="submit-btn" style="margin-bottom:15px;background:#ff9800;font-size:16px;padding:14px 24px" onclick="showAddProduct()">â \u6dfb\u52a0\u5546\u54c1</button>';
  h+='<div class="admin-header"><div class="admin-title">âï¸ \u5546\u54c1\u7ba1\u7406\uff08'+ps.length+'\u4ef6\uff09</div><button class="header-btn" onclick="logoutAdmin()">\u9000\u51fa</button></div>';
  if(ps.length===0){h+='<div class="empty"><span class="empty-icon">ð¦</span><p>\u6682\u65e0\u5546\u54c1</p></div>';}
    ...
    for(var i=0;i<ps.length;i++){
      ...
      h+='</div><div class="admin-item-price">Â¥'+p.price+'</div>';
...
function rebuildForm(){
  ...
  tagH+='</div>';
  var imgH='';
  for(var j=0;j<selectedImages.length;j++){imgH+='<div class="image-preview-item"><img src="'+selectedImages[j]+'" onerror="this.style.display=\'none\'"><button class="delete-btn" onclick="removeImage('+j+')">Ã</button><button class="set-cover-btn" onclick="setCover('+j+')">\u5c01\u9762</button></div>';}
  var vidH='';
  if(selectedVideo || pendingVideoThumb){
    ...
    if(thumbSrc&&thumbSrc.indexOf('data:')===0){
      ...
      vidH+='<div class="video-thumb-placeholder"><div class="video-thumb-icon">ð¬</div><div class="video-thumb-name">'+(vidLabel||'\u5df2\u9009\u62e9\u89c6\u9891')+'</div></div>';
    }
    vidH+='<button class="delete-btn" onclick="removeVideo()">Ã</button></div>';
  }
  var c=document.getElementById('mainContainer');
  c.innerHTML='<div class="modal-overlay show" id="addModal"><div class="modal">'+
    '<div class="modal-header"><h3>'+(isEdit?'âï¸ \u7f16\u8f91\u5546\u54c1':'â \u6dfb\u52a0\u5546\u54c1')+'</h3><button class="modal-close" onclick="closeModal()">Ã</button></div>'+
    ...
    '<div class="form-item"><label class="form-label">\u5546\u54c1\u56fe\u7247\uff08\u6700\u591a9\u5f20\uff0c\u81ea\u52a8\u538b\u7f29\uff09</label>'+imgH+'<div class="upload-area" onclick="document.getElementById(\'imgInput\').click()"><div class="upload-icon">ð·</div><div class="upload-text">\u70b9\u51fb\u4e0a\u4f20\u56fe\u7247</div></div><input type="file" id="imgInput" accept="image/*" multiple style="display:none" onchange="handleImageUpload(this)"></div>'+
    '<div class="form-item"><label class="form-label">\u5546\u54c1\u89c6\u9891\uff08\u53ef\u9009\uff0c\u81ea\u52a8\u63d0\u53d6\u7f29\u7565\u56fe\uff09</label>'+vidH+'<div class="upload-area" onclick="document.getElementById(\'vidInput\').click()"><div class="upload-icon">ð¬</div><div class="upload-text">\u70b9\u51fb\u4e0a\u4f20\u89c6\u9891</div></div><input type="file" id="vidInput" accept="video/*" style="display:none" onchange="handleVideoUpload(this)"></div>'+
    '<button class="submit-btn" onclick="saveProduct()">'+(isEdit?'ð¾ \u4fdd\u5b58\u4fee\u6539':'â \u6dfb\u52a0\u5546\u54c1')+'</button>'+
    '</div></div></div>';
}
