...
async function showDetail(id){
  ...
  var unit=p.unit||'\u4ef6';
  h+='<div class="detail-content">'+backBtn+'<div class="detail-name">'+p.name+'</div>';
  h+='<div style="margin:12px 0"><button class="detail-btn inquiry-btn" onclick="openInquiryForm(\''+p.name+'\','+p.id+','+p.price+',\''+unit+'\')" style="background:#c0392b;color:#fff;border:none;padding:12px 24px;border-radius:8px;font-size:15px;width:100%;cursor:pointer">获取报价</button></div>';
  if(p.tag)h+='<span class="detail-tag">'+p.tag+'</span>';
  if(p.description)h+='<div class="detail-desc">'+p.description+'</div>';
  h+='<div id="inquiryForm" style="display:none;background:#f9f9f9;border-radius:8px;padding:16px;margin-top:12px">';
  h+='<div style="font-size:13px;color:#666;margin-bottom:12px">提交后将由客服联系您，请保持手机畅通</div>';
  h+='<input id="inqName" type="text" placeholder="您的姓名" style="width:100%;padding:10px;border:1px solid #ddd;border-radius:6px;box-sizing:border-box;margin-bottom:8px">';
  h+='<input id="inqPhone" type="tel" placeholder="手机号（必填）" maxlength="11" style="width:100%;padding:10px;border:1px solid #ddd;border-radius:6px;box-sizing:border-box;margin-bottom:8px">';
  h+='<div id="inqError" style="color:#c0392b;font-size:12px;margin-bottom:8px;display:none"></div>';
  h+='<button class="detail-btn" onclick="submitInquiry(\''+p.name+'\','+p.id+','+p.price+',\''+unit+'\')" style="width:100%;background:#c0392b;color:#fff;border:none;padding:12px;border-radius:6px;cursor:pointer;font-size:14px">提交询价</button>';
  h+='</div>';
  h+='<div style="margin-top:15px"><button class="detail-btn copy" onclick="switchTab(\'home\')" style="width:100%">← \u8fd4\u56de\u9996\u9875</button></div></div>';
...
function openInquiryForm(name,id,price,unit){var f=document.getElementById('inquiryForm');if(f){f.style.display='block';window.scrollTo(0,document.body.scrollHeight);}}
function submitInquiry(name,id,price,unit){var phone=document.getElementById('inqPhone').value.trim();var iname=document.getElementById('inqName').value.trim();var err=document.getElementById('inqError');if(err)err.style.display='none';if(!phone){if(err){err.textContent='请输入手机号';err.style.display='block'}return;}if(!/^1[3-9]\d{9}$/.test(phone)){if(err){err.textContent='手机号格式不正确';err.style.display='block'}return;}var btn=document.querySelector('#inquiryForm button');if(btn){btn.disabled=true;btn.textContent='提交中...'};fetch(SUPABASE_URL+'/rest/v1/inquiry_logs',{method:'POST',headers:{'Content-Type':'application/json','apikey':SUPABASE_ANON_KEY,'Authorization':'Bearer '+SUPABASE_ANON_KEY},body:JSON.stringify({product_name:name,product_id:id,price:price,unit:unit,contact_name:iname||'',phone:phone,page:"前台",created_at:new Date().toISOString()})}).then(function(r){return r.ok?r.json():r.text().then(function(t){throw new Error(t)})}).then(function(){var f=document.getElementById('inquiryForm');if(f)f.innerHTML='<div style="text-align:center;padding:20px"><div style="font-size:32px;margin-bottom:8px">✔</div><div style="color:#333">报价已提交，客服将尽快联系您</div></div>'}).catch(function(e){if(err){err.textContent='提交失败：'+e.message;err.style.display='block'}if(btn){btn.disabled=false;btn.textContent='提交询价'}})}
